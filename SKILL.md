...
description: |
  Daily Memory Archiver v1.6.1 — OpenClaw 会话归档：按 session key 统计用量、检查点后仅合并新增消息、可选分块云端摘要、默认仅对超限 key 执行 sessions.compact。推荐根目录 ~/.openclaw/workspace/skills/daily-memory-archiver（自维护）；亦支持 ~/.openclaw/skills/daily-memory-archiver（全局）。
...
**文档与实现版本：1.6.1**（`config.yaml` 中 `skill_version` 可与本文不一致时，以本文与脚本为准。）
